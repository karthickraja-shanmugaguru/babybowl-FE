...
const MobileNavigation = (props) => {
  const { close, mobileMenu } = props;

  const [subMenu, setSubMenu] = useState();
  const [category, setCategory] = useState();
  const [depth, setDepth] = useState(0);
  const [categoryLists, setCategoryLists] = useState([]); //to list categories in shop section in mobileview sidebar
    ...
    <div className={styles.root}>
      <nav>
        ...
        <div className={styles.mobileNavContainer}>
          ...
          {depth === 1 &&
            categoryLists?.map((menuItem) => {
              return (
                <Link
                  key={menuItem?.id}
                  to={'/shop/'}
                  state={{ category: menuItem?.name }}
                  onClick={() => {
                    close();
                    // setDepth(2);
                    // setSubMenu(menuItem);
